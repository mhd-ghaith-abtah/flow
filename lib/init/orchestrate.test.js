...
describe('runInit (dryRun)', () => {
  ...
  it('drops .caveman-enable marker when Caveman subset != none and not dry-run', async () => {
    // Headless parity with the slash path (skills/flow-init/workflow.md step 8b).
    // The slash workflow has always dropped the marker; the headless orchestrator
    // didn't until this test was added. README + usage.md both promise the
    // marker lands automatically — that promise is now true for both paths.
    const cwd = mkdtempSync(join(tmpdir(), 'flow-init-orch-marker-'));
    const homeDir = mkdtempSync(join(tmpdir(), 'flow-init-orch-marker-home-'));
    try {
      // Use the chain helper but explicitly LEAVE cavemanSubset at full (the
      // chainTestAnswers helper forces it to none, which would suppress the
      // marker — exactly the path we don't want here).
      const answers = defaultAnswersForProfile(catalog, 'minimal', {
        bmadSubset: 'none',
        eccSubset: 'none',
        cavemanSubset: 'full',  // ← keep Caveman requested
      });
      const result = await runInit({
        cwd, homeDir, catalog, flowVersion: '0.7.99-test',
        cliAnswers: answers, dryRun: false,
      });
      assert.equal(result.ok, true, result.haltReason);
      assert.equal(existsSync(join(cwd, '.caveman-enable')), true,
        '.caveman-enable should exist after a non-dry-run install when Caveman is requested');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
      rmSync(homeDir, { recursive: true, force: true });
    }
  ...
  it('does NOT drop .caveman-enable when Caveman subset is none', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'flow-init-orch-nomarker-'));
    const homeDir = mkdtempSync(join(tmpdir(), 'flow-init-orch-nomarker-home-'));
    try {
      const answers = chainTestAnswers(catalog, 'minimal');  // forces caveman=none
      const result = await runInit({
        cwd, homeDir, catalog, flowVersion: '0.7.99-test',
        cliAnswers: answers, dryRun: false,
      });
      assert.equal(result.ok, true);
      assert.equal(existsSync(join(cwd, '.caveman-enable')), false,
        '.caveman-enable should NOT exist when cavemanSubset is none');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
      rmSync(homeDir, { recursive: true, force: true });
    }
  ...
  it('does NOT drop .caveman-enable in dry-run mode', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'flow-init-orch-drymarker-'));
    const homeDir = mkdtempSync(join(tmpdir(), 'flow-init-orch-drymarker-home-'));
    try {
      const answers = defaultAnswersForProfile(catalog, 'minimal', {
        bmadSubset: 'none', eccSubset: 'none', cavemanSubset: 'full',
      });
      const result = await runInit({
        cwd, homeDir, catalog, flowVersion: '0.7.99-test',
        cliAnswers: answers, dryRun: true,
      ...
      assert.equal(result.ok, true);
      assert.equal(existsSync(join(cwd, '.caveman-enable')), false,
        'dry-run should not write the marker');
    } finally {
      rmSync(cwd, { recursive: true, force: true });
      rmSync(homeDir, { recursive: true, force: true });
    }
