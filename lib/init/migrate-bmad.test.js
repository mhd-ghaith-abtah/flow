...
describe('migrate', () => {
  it('dryRun=true reports counts without writing files or staging backups', () => {
    const dir = makeTmpProject();
    try {
      seedSprintStatus(dir, SAMPLE_SPRINT);
      writeFileSync(join(dir, DEFERRED_REL), '- alpha\n- beta\n');

      const result = migrate(dir, { dryRun: true });
      ...
      assert.equal(result.stateRecord.dry_run, true);

      // Crucially: nothing written and no backup files created.
      assert.deepEqual(result.backups, []);
      assert.equal(existsSync(join(dir, FLOW_SPRINT_REL)), false, 'sprint.yaml must NOT exist');
      assert.equal(existsSync(join(dir, FLOW_DEFERRED_REL)), false, 'deferred.md must NOT exist');
      assert.equal(existsSync(join(dir, FLOW_STORIES_REL)), false, 'stories dir must NOT exist');
      assert.equal(
        existsSync(join(dir, `${SPRINT_REL}.flow-backup-${result.backupTs}`)),
        false,
