...
describe('flow plan', () => {
  ...
  // E7-002 follow-up: --ecc-scope CLI override.
  it('--ecc-scope=project overrides mini profile default (user → project)', async () => {
    const { rc, stdout } = await captureOutput(() =>
      plan({ profile: 'mini', json: true, 'ecc-scope': 'project' }, ctx)
    );
    assert.equal(rc, 0);
    const parsed = JSON.parse(stdout);
    assert.equal(parsed.upstreams.ecc_install_scope, 'project');
  });

  it('--ecc-scope=user overrides team profile default (project → user)', async () => {
    const { rc, stdout } = await captureOutput(() =>
      plan({ profile: 'team', json: true, 'ecc-scope': 'user' }, ctx)
    );
    assert.equal(rc, 0);
    const parsed = JSON.parse(stdout);
    assert.equal(parsed.upstreams.ecc_install_scope, 'user');
  });

  it('no --ecc-scope keeps the profile default', async () => {
    const { rc, stdout } = await captureOutput(() =>
      plan({ profile: 'team', json: true }, ctx)
    );
    assert.equal(rc, 0);
    const parsed = JSON.parse(stdout);
    assert.equal(parsed.upstreams.ecc_install_scope, 'project');
  ...
  it('--ecc-scope with invalid value throws so typos do not silently fall through', async () => {
    await assert.rejects(
      () => plan({ profile: 'mini', json: true, 'ecc-scope': 'projet' }, ctx),
      /--ecc-scope must be "user" or "project"/
    );
  });
});
