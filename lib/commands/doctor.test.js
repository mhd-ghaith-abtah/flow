...
describe('flow doctor', () => {
  ...
  it('finds flow.config.yaml at canonical .claude/flow.config.yaml path', async () => {
    // Scaffold writes here. doctor should look here first per the
    // canonical-vs-legacy probe order.
    const canonicalDir = join(tmpDir, '.claude');
    mkdirSync(canonicalDir, { recursive: true });
    const canonicalPath = join(canonicalDir, 'flow.config.yaml');
    writeFileSync(canonicalPath, 'version: 1\nmode: minimal\nadapters:\n  issue_tracker: none\n  pr: none\n  e2e: none\n  verify: custom\n');
    try {
      const { rc, stdout } = await captureOutput(() => doctor({}, ctx()));
      // Doctor should report Config ✓ pointing at the canonical path.
      assert.match(stdout, /\.claude\/flow\.config\.yaml/);
      // No warnings expected from this case alone.
      assert.notEqual(rc, 2);
    } finally { rmSync(canonicalDir, { recursive: true, force: true }); }
  });

  it('falls back to legacy root flow.config.yaml when canonical path absent', async () => {
    // Pre-canonical installs put flow.config.yaml at project root.
    // Doctor should still find it via the fallback path.
    const legacyPath = join(tmpDir, 'flow.config.yaml');
    writeFileSync(legacyPath, 'version: 1\nmode: minimal\nadapters:\n  issue_tracker: none\n  pr: none\n  e2e: none\n  verify: custom\n');
    try {
      const { stdout } = await captureOutput(() => doctor({}, ctx()));
      // Report should reference the legacy path since it's what exists.
      assert.match(stdout, new RegExp(legacyPath.replace(/\//g, '\\/').replace(/\./g, '\\.')));
    } finally { rmSync(legacyPath, { force: true }); }
  });

  it('canonical path wins when both canonical AND legacy paths exist', async () => {
    // Edge case: user manually placed a legacy flow.config.yaml AND ran a
    // new install that wrote the canonical one. Canonical is the source of
    // truth — doctor should report it, not the legacy stub.
    const canonicalDir = join(tmpDir, '.claude');
    mkdirSync(canonicalDir, { recursive: true });
    const canonicalPath = join(canonicalDir, 'flow.config.yaml');
    const legacyPath = join(tmpDir, 'flow.config.yaml');
    writeFileSync(canonicalPath, 'version: 1\nmode: minimal\nadapters:\n  issue_tracker: none\n  pr: none\n  e2e: none\n  verify: custom\n');
    writeFileSync(legacyPath, 'version: 1\nmode: minimal\n');  // missing adapters — would warn
    try {
      const { rc, stdout } = await captureOutput(() => doctor({}, ctx()));
      // Should be reading the canonical (valid) config, NOT warning about the legacy stub.
      assert.match(stdout, /\.claude\/flow\.config\.yaml/);
      assert.doesNotMatch(stdout, /missing required 'adapters' section/);
      assert.notEqual(rc, 2);
    } finally {
