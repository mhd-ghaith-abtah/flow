...
function renderHuman(p, ctx) {
  ...
  lines.push('');
  lines.push(chalk.dim('Preview only. Execute via:'));
  lines.push(chalk.dim('  flow init --profile ' + p.profile + ' --yes        # headless'));
  lines.push(chalk.dim('  /flow-init    (inside Claude Code)                 # interactive'));
